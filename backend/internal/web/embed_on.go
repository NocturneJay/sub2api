...
// writeIndexHTML 写出 index.html，并显式声明 Content-Length。
//
// ⚠️ AICat 自研改动（2026-08-12）。同步上游时请保留——上游是直接调 c.Data 的。
//
// 为什么不能直接用 c.Data：Go 的 net/http 对超过内部嗅探缓冲的响应体默认走
// Transfer-Encoding: chunked。而 Caddy 的规则是「Content-Length 未知即视为流式
// 响应，每次写入后立即 flush」（见 caddyserver.com 的 reverse_proxy 文档，
// flush_interval 小节）。逐块 flush 会反复打断压缩器的滑动窗口，使边缘 encode
// 指令的压缩率崩塌。
//
// 实测数据（api-cn.aicatstudios.com，381309 字节的 index.html，其中 333096 字节
// 即 87% 是 base64 大块，高度依赖大窗口压缩）：
//
//	chunked + Caddy gzip 6  -> 296660 字节（1.29x）
//	chunked + Caddy zstd    -> 330034 字节（1.16x，比 gzip 还差）
//	离线一次性 zstd -3      -> 137828 字节（2.77x）
//	（对照：Cloudflare 边缘 br -> 137909 字节，与离线 zstd 几乎相同）
//
// 显式声明 Content-Length 后 Caddy 会完整缓冲再压缩，可取回上面那约 2 倍的差额。
// 同一部署里带 Content-Length 的静态资源（assets/*.js: 181271 -> 55877，3.24x）
// 压缩率一直正常，是这个判断的对照组。
//
// 安全性：len(content) 就是紧接着写入的确切字节数，不存在与实际 body 不符的风险。
// SSE / 流式响应不经过这个函数，不受影响。
func writeIndexHTML(c *gin.Context, content []byte) {
