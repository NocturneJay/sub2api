...
func (s *GatewayService) SelectAccountWithLoadAwareness(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, metadataUserID string, sub2apiUserID int64) (*AccountSelectionResult, error) {
	...
	// 利润管控闸门必须装在委托解析之后，用委托后的子分组。上游没有复合分组委托
	// 这一层，其原始位置在委托之前。若沿用上游顺序，复合分组会用父分组去装闸门，
	// 而父分组平台是 composite、不在 profitControlPlatformSupported 之列，闸门会被
	// 直接清掉——子分组上配的利润管控就静默失效了。委托后再装还有一个好处：
	// withGroupContext 已把子分组写进 ctxkey.Group，正是闸门取计费分组的兜底来源。
	ctx = s.withGatewayProfitControlGate(ctx, groupID)
