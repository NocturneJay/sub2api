...
const (
	...
	SettingKeyRedeemPurchaseURL           = "redeem_purchase_url"           // 兑换码购买页面 URL（在新窗口打开）
	...
	// 以下三个键属上游自带的模型广场。aicat 使用自研广场、不路由到上游前端，
	// 保留这些键是为了让上游后端与设置链路完整，避免每次同步都要拆改。
	// 它们与上面 aicat 的 model_plaza_public_* 键语义独立、互不影响。
