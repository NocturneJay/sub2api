...
func (s *OpenAIGatewayService) forwardOpenAIPassthrough(
	...
	if reqStream {
		result, err := s.handleStreamingResponsePassthroughWithImageIntent(ctx, resp, c, account, startTime, reqModel, upstreamPassthroughModel, imageIntent)
		if err != nil {
			return nil, err
		}
		usage = result.usage
...
func (s *OpenAIGatewayService) handleStreamingResponsePassthrough(
	...
	imageIntent := OpenAIImageGenerationIntentFromContext(ctx) ||
		isOpenAIImageGenerationModel(originalModel) ||
		isOpenAIImageGenerationModel(mappedModel)
	return s.handleStreamingResponsePassthroughWithImageIntent(
		ctx, resp, c, account, startTime, originalModel, mappedModel, imageIntent,
	)
}

func (s *OpenAIGatewayService) handleStreamingResponsePassthroughWithImageIntent(
	ctx context.Context,
	resp *http.Response,
	c *gin.Context,
	account *Account,
	...
	// 上游 v0.1.172 的「上游响应模型审计」观察器要装在**真正的函数体**里，
	// 不能只装在上面那个薄包装里：aicat 把这个函数拆成了两层，而
	// openai_gateway_passthrough.go:248 那条路径是**直接调 WithImageIntent 版**的，
	// 装在包装层会让那条路径漏采。
	...
	// 注意：scanBuf 的归还不在此处 defer——同步分支与读取协程分支各自负责归还，
	// 函数级 defer 会造成双重归还污染池。
		...
		if data, ok := extractOpenAISSEDataLine(line); ok {
			...
			if eventType == "error" && !openAIStreamClientOutputStarted(c, clientOutputStarted) {
				...
				if status, errType, errMsg, matched := applyOpenAIStreamFailedErrorPassthroughRule(c, account.Platform, dataBytes, errorMessage); matched {
					...
					// aicat：processLine 是不返回值的闭包（keepalive 写法），上游那句
					// `return resultWithUsage(), err` 在这里编译不过。改经 streamFailoverErr
					// 上抛，出口处照样返回 resultWithUsage() + 该错误，语义与上游一致。
					// 与本函数下方 response.failed 分支的处理方式相同。
					streamFailoverErr = fmt.Errorf("upstream error event: passthrough rule matched message=%s", errMsg)
					return
				}
				if openAIStreamErrorEventShouldFailover(dataBytes, errorMessage) {
					streamFailoverErr = s.newOpenAIStreamFailoverError(c, account, true, upstreamRequestID, dataBytes, errorMessage, resp.Header)
					return
				}
			}
			if eventType == "response.failed" {
				...
				if !openAIStreamClientOutputStarted(c, clientOutputStarted) {
					...
					if openAIStreamFailedEventShouldFailover(dataBytes, failedMessage) {
						// 保留 aicat 的 keepalive 闭包写法(经由 streamFailoverErr 上抛，
						// 不能在 processLine 里直接 return 结果)，并采纳上游新增的
						// resp.Header 实参——该形参是 variadic，其余调用点无需同步修改。
						streamFailoverErr = s.newOpenAIStreamFailoverError(c, account, true, upstreamRequestID, dataBytes, failedMessage, resp.Header)
						return
					}
				}
			...
			if (eventType == "response.completed" || eventType == "response.done") &&
				...
				// 同上：aicat 的 keepalive 闭包写法里 processLine 不返回值，上游那句
				// `return resultWithUsage(), newOpenAI…` 在这里编译不过。改经
				// streamFailoverErr 上抛，出口处（scan 结束后的两处）会照样返回
				// resultWithUsage() + 该 failover 错误，语义与上游一致。
				streamFailoverErr = newOpenAIResponsesEmptyCompletedFailoverError(c, account, upstreamRequestID)
				return
			}
	...
	// keepalive：图片生成等场景上游可长时间不吐数据，需持续向下游写心跳字节，
	// 否则中间层代理（如 Cloudflare 免费版约 100s 空闲超时）会以 524 掐断连接。
	keepaliveInterval := time.Duration(0)
	if imageIntent {
		keepaliveInterval = s.openAIImageStreamKeepaliveInterval()
	} else if s.cfg != nil && s.cfg.Gateway.StreamKeepaliveInterval > 0 {
		keepaliveInterval = time.Duration(s.cfg.Gateway.StreamKeepaliveInterval) * time.Second
	}

	// 无 keepalive 时走同步扫描，减少 goroutine 与 channel 开销。
	if keepaliveInterval <= 0 {
		defer putSSEScannerBuf64K(scanBuf)
		for documentScanner.Scan() {
			processLine(documentScanner.Text())
			if streamFailoverErr != nil {
	...
	// 独立 goroutine 读取上游，使上游长时间静默时 keepalive 仍能触发
	...
	for {
		...
		case <-keepaliveTicker.C:
			// 心跳只在 clientOutputStarted 后发送：pre-output 阶段 pendingLines
			// 仍在等待 failover 判定，提前写出任何字节都会提交响应状态、破坏换号重试。
