...
func (s *OpenAIGatewayService) forwardOpenAIPassthrough(
	...
	for {
		...
		if reqStream {
			// aicat：显式传本函数已判定过的 imageIntent（含 attemptImageIntentInvalidated
			// 失效重算），不走薄包装从 ctx 兜底推导——失效场景下两者会分歧。
			result, handleErr := s.handleStreamingResponsePassthroughWithImageIntent(ctx, resp, c, account, startTime, reqModel, upstreamPassthroughModel, imageIntent)
			if handleErr != nil {
...
func (s *OpenAIGatewayService) handleStreamingResponsePassthroughWithImageIntent(
	...
	// aicat：processLine 是不返回值的闭包（keepalive 写法），上游的扫描循环里那些
	// `return resultWithUsage(), err` 在这里编译不过，一律改经 streamFailoverErr 上抛，
	// 出口处（两个分支的 scan 结束点）照样返回 resultWithUsage() + 该错误，语义与上游一致；
	// 上游循环里的 `continue` 对应改为 `return`（结束当前行处理）。
		...
		if data, ok := extractOpenAISSEDataLine(line); ok {
			...
			if eventType == "response.failed" || eventType == "error" {
				...
				if !outputStarted {
					...
					if shouldFailover {
						// aicat：processLine 是不返回值的闭包（keepalive 写法），上游的
						// `return resultWithUsage(), err` 形状必须经 streamFailoverErr 上抛
						//（规约 v0.1.176 第 3 条同族）。函数体采上游 v0.1.184 的
						// WithModel 版（failover 保留模型作用域，804679d99）。
						streamFailoverErr = s.newOpenAIStreamFailoverErrorWithModel(c, account, true, upstreamRequestID, dataBytes, failedMessage, mappedModel, resp.Header)
						return
					}
	...
	if keepaliveInterval <= 0 {
		...
		// 上游 v0.1.182：codex 裸 error 未跟 response.failed 就 EOF 时，
		// 在错误判定之前合成一帧 response.failed 终止事件（两个分支同此）。
