...
func TestComputeBusinessHealth(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "TTFT within 8s full marks",
			overview: &OpsDashboardOverview{
				SLA:               0.99,
				ErrorRate:         0,
				UpstreamErrorRate: 0,
				TTFT:              OpsPercentiles{P99: intPtr(7999)},
			},
			wantMin: 100,
			wantMax: 100,
		},
		{
			name: "TTFT midpoint 11.5s",
			overview: &OpsDashboardOverview{
				SLA:               0.99,
				ErrorRate:         0,
				UpstreamErrorRate: 0,
				TTFT:              OpsPercentiles{P99: intPtr(11500)},
			},
			wantMin: 75,
			wantMax: 75,
		},
		{
			name: "TTFT above 15s zero",
			overview: &OpsDashboardOverview{
				SLA:               0.99,
				ErrorRate:         0,
				UpstreamErrorRate: 0,
				TTFT:              OpsPercentiles{P99: intPtr(16000)},
			},
			wantMin: 50,
			wantMax: 50,
		},
