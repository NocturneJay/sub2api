...
func (s *GatewayService) recordUsageCore(ctx context.Context, input *recordUsageCoreInput, opts *recordUsageOpts) error {
	...
	// 计算费用
	// 采纳上游新增的 pricingAt 形参；定价 key 仍是委托后的 pricingAPIKey（子分组）。
	cost := s.calculateRecordUsageCost(ctx, result, pricingAPIKey, billingModel, multiplier, imageMultiplier, pricingAt, opts)
	...
	if responseModel := responseModelBillingDeclaration(
		...
		if identified, responseChannelPriced := s.hasIdentifiedResponseModelPricing(ctx, responseModel, pricingAPIKey); identified {
			responseCost := s.calculateRecordUsageCost(ctx, result, pricingAPIKey, responseModel, multiplier, imageMultiplier, pricingAt, opts)
