...
func (s *GatewayService) recordUsageCore(ctx context.Context, input *recordUsageCoreInput) error {
	...
	// token 倍率叠加高峰因子（token 计费含图片 token，图片按次倍率不受影响）。高峰因子按请求的
	// 定价时刻现算，不并入上面的 getUserGroupRateMultiplier，以免污染 user:group 倍率缓存。
	//
	// 两处与上游不同：
	//   1. 高峰策略取自委托后的定价 API key —— 复合分组按目标子分组定价，高峰窗口
	//      也必须跟着子分组走，否则会拿父分组的峰谷策略去套子分组的价。
	//   2. 时刻用 input.PricingAt（上游本区间引入）而非计费时的 timezone.Now()：
	//      定价时刻一经固定，长请求就不会因为计费发生在高峰边界另一侧而算错价。
	...
	multiplier, imageMultiplier := computePeakAwareMultipliers(pricingPolicyAPIKey, baseMultiplier, pricingAt)
	...
	// 计算费用
	// 采纳上游 v0.1.185 去掉 opts 形参的新签名（长上下文入口规则随目录数据驱动化退役）；
	// 定价 key 仍是委托后的 pricingAPIKey（子分组，规约冲突登记 v0.1.176 第 1 条）。
	cost := s.calculateRecordUsageCost(ctx, result, pricingAPIKey, billingModel, multiplier, imageMultiplier, pricingAt)
	...
	//
	// aicat：三处定价查表全部走 pricingAPIKey（委托后的子分组），与上面的基线 cost
	// 保持同一定价维度；用父分组的 apiKey 会让复合分组的响应模型计费按父分组查表。
	if responseModel := responseModelBillingDeclaration(
		...
		if identified, responseChannelPriced := s.hasIdentifiedResponseModelPricing(ctx, responseModel, pricingAPIKey); identified {
			responseCost := s.calculateRecordUsageCost(ctx, result, pricingAPIKey, responseModel, multiplier, imageMultiplier, pricingAt)
			baselineChannelPriced := s.resolveChannelPricing(ctx, billingModel, pricingAPIKey) != nil
...
// billingPricingGroup 返回与 billingPricingGroupID 同一维度的分组对象，供上游
// v0.1.176 的分组逐模型定价（Group.ModelPricing / LongContextPricingEnabled）查表。
// 两者不一致时返回 nil：宁可退回「无分组价卡」的既有行为，也不能拿复合父分组的
// 价卡去套子分组的渠道查表。委托路径上调用方传的是 pricingAPIKey（Group 已是子分组），
// 因此正常情况下必然一致。
func billingPricingGroup(apiKey *APIKey, pricingGroupID int64) *Group {
	if apiKey == nil || apiKey.Group == nil || apiKey.Group.ID != pricingGroupID {
		return nil
	}
	return apiKey.Group
}
...
func (s *GatewayService) resolveChannelPricing(ctx context.Context, billingModel string, apiKey *APIKey) *ResolvedPricing {
	...
	resolved := s.resolver.Resolve(ctx, PricingInput{
		Model: billingModel, GroupID: &gid, Group: billingPricingGroup(apiKey, gid),
	})
...
func (s *GatewayService) calculateTokenCost(
	...
	// aicat：定价预解析必须走委托后的分组维度（billingPricingGroupID / billingPricingGroup），
	// 不能像上游那样直接取 apiKey.Group.ID——复合分组委托路径上带 delegated ctx 而未克隆的
	// 调用方会拿父分组的价卡去套子分组的渠道查表（v0.1.176 冲突登记第 1/2 条）。
	// 路径选择本身采上游 v0.1.182 的统一入口 CalculateTokenCostForRequest。
