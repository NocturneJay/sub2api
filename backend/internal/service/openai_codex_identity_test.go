...
func TestBuildCodexCLIUserAgent(t *testing.T) {
	// codex-rs 现行形态：OS/架构/终端后缀之后还有 `(name; version)` 官方客户端标识组。
	require.Equal(t, openai.CodexDefaultOriginator+"/0.200.1"+codexCLIUserAgentSuffix+codexUATrailer(openai.CodexDefaultOriginator, "0.200.1"), buildCodexCLIUserAgent("0.200.1"))
