...
func (s *OpenAIGatewayService) RecordUsage(ctx context.Context, input *OpenAIRecordUsageInput) error {
	...
	// 上游 v0.1.184:计费凭据账号(shadow credential tier contract)。
	...
	//
	// 与上游的差别：高峰策略与 video 独立倍率都取自委托后的定价 API key。
	// 复合分组按目标子分组定价，峰谷窗口和 video 倍率必须跟着子分组走，
	// 否则会拿父分组的策略去套子分组的价。上游没有复合分组委托这一层。
	baseMultiplier := multiplier
	// 采纳上游把 pricingAt 提成局部变量（下面新增的逻辑要复用它），但**峰谷策略与
	// video 倍率必须仍取委托后的定价 API key**：复合分组按目标子分组定价，用父分组
	// 的策略去套子分组的价是静默算错账。上游没有复合分组委托这一层。
...
func (s *OpenAIGatewayService) calculateOpenAIVideoCost(
	...
	if resolved != nil && resolved.Source == PricingSourceChannel &&
		...
		cost, err := s.billingService.CalculateCostUnified(CostInput{
			...
			Model:          billingModel,
			GroupID:        &gid,
			Group:          billingPricingGroup(apiKey, gid),
