...
func (h *OpenAIGatewayHandler) ResponsesInputTokens(c *gin.Context) {
	...
	// 上游本轮新增的这个入口是「替换式」的——直接拿映射后的名字去选号（下面的
	// SelectAccountWithSchedulerForCapability 原本传 routingModel），那样只有声明了
	// 映射后名字的账号能被选中。改成与其余 12 个入口一致的「加法」口径：选号传原始名，
	// 由别名兜住映射后名字，两种账号配置都能选到（严格超集，不会让原本可选的落选）。
	...
	// aicat：选号传原始名（上游传 routingModel 即映射名，是「替换式」口径），
	// 映射后名字由上面挂进 ctx 的渠道路由别名兜住——与其余入口一致的「加法」口径。
