...
func (h *PaymentHandler) GetPlans(c *gin.Context) {
	...
	type planWithPlatform struct {
		...
		RoutePricing       []service.CompositeRoutePricingInfo `json:"composite_route_pricing,omitempty"`
	...
	for _, p := range plans {
		...
		peakEnabled, peakStart, peakEnd, peakMultiplier := planPeakRate(gi)
		result = append(result, planWithPlatform{
			ID: int64(p.ID), GroupID: p.GroupID,
			GroupPlatform: gi.Platform, GroupName: gi.Name,
			RateMultiplier: planRateMultiplier(gi), RoutePricing: gi.CompositeRoutePricing,
			PeakRateEnabled: peakEnabled,
			PeakStart:       peakStart, PeakEnd: peakEnd, PeakRateMultiplier: peakMultiplier,
...
func (h *PaymentHandler) GetCheckoutInfo(c *gin.Context) {
	...
	for _, p := range plans {
		...
		planList = append(planList, checkoutPlan{
			ID: int64(p.ID), GroupID: p.GroupID,
			GroupPlatform: gi.Platform, GroupName: gi.Name,
			RateMultiplier:  planRateMultiplier(gi),
			RoutePricing:    gi.CompositeRoutePricing,
			PeakRateEnabled: peakEnabled, PeakStart: peakStart,
			PeakEnd: peakEnd, PeakRateMultiplier: peakMultiplier,
...
func planRateMultiplier(info service.PlanGroupInfo) *float64 {
...
func planPeakRate(info service.PlanGroupInfo) (bool, string, string, float64) {
	if info.Platform == service.PlatformComposite {
		return false, "", "", 0
	}
	return info.PeakRateEnabled, info.PeakStart, info.PeakEnd, info.PeakRateMultiplier
}
