...
// aicat 的 f1404cc15 曾在这里放两个用例，钉死「只有严格官方 Codex UA 才能从 body
// 信号推断远程压缩 v2；第三方客户端一律降级到 legacy /responses/compact」。
//
// **上游 v0.1.177 用更好的规则取代了它**（9662cff2e + a8b9ea22b + 8ae6d8f67）：
// 协议由报文本身决定——流式 + compaction_trigger 就是 v2 线路，与客户端是谁无关。
// 降级成 legacy 一元端点会剥掉 stream，任何客户端都会坏，UA 门只是把这个坏处
// 限定在第三方客户端身上而已，并不是更安全。
//
// 因此这两个用例**已随 aicat 那处 UA 门一起退役**，不要因为「上游合并时丢了
// 自研用例」的直觉把它们加回来——它们断言的正是被刻意消除的行为。
// 覆盖同一场景的是上游的 TestOpenAIResponsesCompactionRoutingFlags
// （native_v2_stream_trigger / stream_false_promotes / stream_absent_promotes）
// 与 TestNormalizeOpenAIResponsesCompactRequest_RemoteV2StaysOnResponses 的
// headerless 分支，前者按 stream 取值分流、后者不带任何头也必须留在原生线路。
