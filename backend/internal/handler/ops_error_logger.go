...
const (
	...
	// opsRequestStartKey 中间件在请求开始时挂上的时刻，供脱离中间件作用域的
	// 落库函数（logOpsStreamErrorValue / logOpsRecoveredUpstream）计算 DurationMs。
	// aicat 0d625fef5「错误看板四项观测失真」修复的一部分，上游没有 DurationMs。
	opsRequestStartKey = "ops_request_start"
...
func OpsErrorLoggerMiddleware(ops *service.OpsService) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		c.Set(opsRequestStartKey, requestStart)
...
func logOpsRecoveredUpstream(c *gin.Context, ops *service.OpsService, finalStatus int) {
	...
	// aicat：唯一入队口是三参版（ctx 用于渠道监控标记 IsChannelMonitor）。
	enqueueOpsErrorLog(requestCtx, ops, entry)
}
...
// opsDurationMsFromContext 从中间件挂上的 opsRequestStartKey 计算总耗时；
// 缺失（非中间件路径进入）时返回 nil，DurationMs 落 NULL。
func opsDurationMsFromContext(c *gin.Context) *int64 {
	if v, ok := c.Get(opsRequestStartKey); ok {
		if start, ok := v.(time.Time); ok {
			return opsDurationMsSince(start)
		}
	}
	return nil
}
