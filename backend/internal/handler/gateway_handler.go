...
func (h *GatewayHandler) Messages(c *gin.Context) {
	...
	if policyBody, changed, err := applyAnthropicReasoningEffortPolicyForGroup(c, apiKey, requestGroup, body); err != nil {
...
func (h *GatewayHandler) Models(c *gin.Context) {
	...
	if platform == service.PlatformComposite {
		...
		if apiKey != nil && apiKey.Group != nil && apiKey.Group.ModelAllowlistEnabled() {
			// aicat：复合可用模型按显式路由枚举、fail-closed，空列表不回填平台默认清单
			//（上游此处会回填 defaultModelIDsForPlatform(composite)，正是被刻意消除的「猜」）。
