...
func TestCompositeRouteTargetPlatform_AllowsCNProviders(t *testing.T) {
	for _, platform := range []string{"kimi", "zhipu", "deepseek"} {
		...
		// aicat 与上游的差别：target_group_id 是必填（复合路由必须委托到具体目标分组），
		// 上游那版没有这个字段、改把 target_platform 设为必填。用例意图（CN 三家应被
		// target_platform 枚举接受）不变，只是补上 aicat 的必填项。
		body := fmt.Sprintf(`{"public_model":"m","target_platform":%q,"target_group_id":1}`, platform)
		require.NoError(t, bindGroupPlatformJSON(t, &req, body))
		require.Equal(t, platform, req.TargetPlatform)
		require.NotNil(t, req.TargetGroupID)
	}
}

// 反向钉死 aicat 分歧：缺 target_group_id 必须被拒（上游允许省略）。
func TestCompositeRouteRequiresTargetGroupID(t *testing.T) {
	var req CompositeRouteRequest
	require.Error(t, bindGroupPlatformJSON(t, &req,
		`{"public_model":"m","target_platform":"openai"}`),
		"复合路由缺少 target_group_id 应被拒绝")
}
