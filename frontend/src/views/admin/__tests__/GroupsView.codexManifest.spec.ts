...
// aicat：上游 v0.2.2 起 GroupsView 在 setup 里读 useAuthStore()（简单模式开关），这份上游 spec
// 没有 mock auth store，无 pinia 时直接抛 getActivePinia；按 columnSettings/duplicate 两份 spec 的写法补上。
vi.mock("@/stores/auth", () => ({
  useAuthStore: () => ({ isSimpleMode: false, isAdmin: true, user: null }),
}));
