export default {
    settings: {
      ...
      features: {
        ...
        affiliate: {
          ...
          firstOrder: {
            ...
            description: 'Every invitee holds a first-recharge voucher from signup. The invitee side needs the first delivered on-site order to reach the threshold, otherwise the voucher is voided on the spot and never reissued. The inviter side ignores the threshold: it pays a share of the invitee first-order amount up to a cap, and already includes the regular percentage rebate.',
            enabled: 'Enable First-Order Bonus',
            enabledHint: 'Requires Enable Affiliate above to be on as well. When off, no first order is evaluated and no bonus is granted; records already written are kept.',
            threshold: 'First-Order Threshold (USD)',
            thresholdDesc: 'The invitee’s first delivered on-site order must reach this amount. Balance orders count the credited amount, subscription orders count the plan price. Changes take effect immediately for every user who has not placed a first order yet.',
            inviteeBonus: 'Invitee Bonus (USD)',
            inviteeBonusDesc: 'Credited straight to the invitee balance once the threshold is met. 0 = none. Changes take effect immediately for every user who has not placed a first order yet.',
            inviterRate: 'Inviter First-Order Rate (%)',
            inviterRateDesc: 'The inviter TOTAL rate on an invitee first order, regardless of the threshold: it still pays when the first order falls short. Whatever the regular percentage rebate already granted is subtracted, and only a positive remainder is added, so the inviter never ends up below the regular rebate. 0 = none, max 100.',
            inviterCap: 'Inviter First-Order Cap (USD)',
            inviterCapDesc: 'The total computed from the rate above is capped at this amount. 0 = none (not "uncapped"). This quota is excluded from the per-invitee rebate cap. Changes take effect immediately for every user who has not placed a first order yet.',
            validDays: 'Voucher Validity (days)',
            validDaysDesc: 'Counted from the moment the invite relationship is created (invitee signup); after it expires the first order earns nothing. 0 = never expires, max 3650. Changes take effect immediately for every user who has not placed a first order yet.',
          },
          customUsers: {
      ...
      site: {
        ...
        redeemPurchaseUrlPlaceholder: 'https://example.com/redeem-codes',
        redeemPurchaseUrlHint: 'HTTPS only. Leave empty to hide the purchase button on the redeem page.',
        redeemPurchaseUrlInvalid: 'Redeem code purchase URL must be an absolute HTTPS URL.',
        siteLogo: 'Site Logo',
        uploadImage: 'Upload Image',
        remove: 'Remove',
        logoHint: 'Uploads are stored as Base64. To keep the home page HTML small, use the image URL field below.',
        logoUrl: 'Image URL (recommended)',
        logoUrlPlaceholder: '/brand/cat-magic-logo.webp or https://cdn.example.com/logo.webp',
        logoUrlHint: 'Supports a same-site path starting with / or an HTTPS URL. It is used for both the site logo and favicon after saving.',
      ...
      customMenu: {
        ...
        description: 'Add embedded pages or new-tab links to the sidebar navigation. Each item can be visible to regular users or administrators.',
        ...
        openMode: 'Open Mode',
        openModeIframe: 'Embed in app (iframe)',
