    ...
    settings: {
      ...
      features: {
        ...
        modelPlazaPublic: {
          enabled: '模型广场允许未登录访问',
          enabledHint:
            '开启后任何人无需登录即可查看模型广场。这会把各分组的折扣倍率、峰谷策略与逐模型完整定价公开到互联网（包括同行）。专属分组任何情况下都不会对匿名访客展示。需先启用上方「可用渠道」，否则广场对登录与匿名一律不可用。默认关闭。',
        ...
        affiliate: {
          ...
          firstOrder: {
            title: '首单双向奖励（首充礼）',
            description: '被邀请人注册时自动持有一张首充券。被邀请人那份要首笔站内支付订单达到阈值才发，不满则当场作废、后续充值不补；邀请人那份不看阈值，按好友首单金额的比例发、封顶一个金额，且已含常规比例返利。',
            enabled: '启用首单双向奖励',
            enabledHint: '需要同时开启上方的「启用邀请返利」才会生效。关闭后不再判定首单、不再发放奖励，已发放的记录保留。',
            threshold: '首单金额阈值（美元）',
            thresholdDesc: '被邀请人首笔已交付的站内订单达到该金额才发奖；余额订单看到账额、订阅订单看套餐价。修改会立即影响所有尚未下首单的用户。',
            inviteeBonus: '被邀请人奖励（美元）',
            inviteeBonusDesc: '达标时直接计入被邀请人余额。0 = 不发。修改会立即影响所有尚未下首单的用户。',
            inviterRate: '邀请人首单返利率（%）',
            inviterRateDesc: '邀请人在好友首单上的**总**返利率，不看阈值：好友首单不满阈值时也照此发放。常规比例返利已发的部分会从中扣除，差额为正才额外计入返利额度，因此邀请人到手永远不低于常规返利。0 = 不发，最大 100。',
            inviterCap: '邀请人首单返利封顶（美元）',
            inviterCapDesc: '上面那个比例算出来的总返利最多给到这个数，超出部分不发。0 = 不发（不是「不封顶」）。这份额度不占用「单人返利上限」。修改会立即影响所有尚未下首单的用户。',
            validDays: '首充券有效期（天）',
            validDaysDesc: '从建立邀请关系（被邀请人注册）起算，超期后首单不再发奖。0 = 永久有效，最大 3650。修改会立即影响所有尚未下首单的用户。',
          },
          customUsers: {
            title: '专属用户配置',
      ...
      site: {
        ...
        redeemPurchaseUrlHint: '仅支持 HTTPS。留空则隐藏兑换页的购买按钮。',
        redeemPurchaseUrlInvalid: '兑换码购买链接必须是完整的 HTTPS 地址。',
        siteLogo: '站点Logo',
        uploadImage: '上传图片',
        remove: '移除',
        logoHint: '上传会把图片存为 Base64；为减小首页 HTML，推荐使用下方的图片 URL。',
        logoUrl: '图片 URL（推荐）',
        logoUrlPlaceholder: '/brand/cat-magic-logo.webp 或 https://cdn.example.com/logo.webp',
        logoUrlHint: '支持以 / 开头的站内路径或 HTTPS 外链。保存后会同时用于站点 Logo 和浏览器图标。',
      ...
      customMenu: {
        title: '自定义菜单页面',
        description: '添加自定义页面或新窗口链接到侧边栏导航。每个菜单可以设置为普通用户或管理员可见。',
        itemLabel: '菜单项 #{n}',
        name: '菜单名称',
